...
```
#add in image of map with sample locations found in the supplementary information

#####Now we are going to transform that data from compositional data to isometric log-ratio coordinates. This is done because normal statistics cannot be run on compositional data. The ilr transformation changes the data into a form that can be clustered. 
```{r}
...
##K-Means Analysis
#####The replication above, though mostly true to what was done within the paper does not any inferential analysis (for example a t-test). Before attempting to use a generalized linear model to analyze the data provided, I decided to perform a K-means analysis. In the paper they also performed a K-means analysis, however their instruction do not show their code for this portion of the paper, because their argument is that the Bayesian models work better for this particular data set. 

#####In the discussion section of their paper they outline their K-means clustering as follows: it uses 22 principle components, and uses four clusters. This removes the need to test for the number of clusters to use. 

#####Load {NbClust} as used in Module 25
```{r}
library(NbClust)
```
...
#####However again, this method does not actually *test* anything. To attempt that I will need to use a generalized linear model or some other modeling technique, go outside of the replication itself, and try to test some kind of relationship. 
